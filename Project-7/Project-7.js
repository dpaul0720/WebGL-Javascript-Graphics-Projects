...
var materialAmbient = vec4( 1.0, 0.8, 0.25, 1.0 );
var materialDiffuse = vec4( 1.0, 0.8, 0.25, 1.0 );
...
window.onload = function init()
{
    ...
    u_SamplerLoc = gl.getUniformLocation(program, "u_Sampler");
    u_SamplerNormalLoc = gl.getUniformLocation(program, "u_SamplerNormal");
    ...
    usebutton = gl.getUniformLocation(program, "u_button");
    ...
    var tick = function() {   // Start drawing
        ...
        if (document.getElementById("Text").checked == true){
            gl.uniform1i(usebutton, 0);
        }
        if (document.getElementById("TextLight").checked == true){
            gl.uniform1i(usebutton, 1);
        }
        if (document.getElementById("TextLightNorm").checked == true){
            gl.uniform1i(usebutton, 2);
        }
...
function initTextures(gl) {
    ...
    colorImage.onload = function(){ loadTexture(gl, colorTexture, u_SamplerLoc, colorImage, 0); };
    ...
    // Register the event handler to be called on loading an image
    normalImage.onload = function(){ loadTexture(gl, normalTexture, u_SamplerNormalLoc, normalImage, 1); };
