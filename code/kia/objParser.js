    ...
    while ((line = lines[index++]) != null) {
        ...
        switch(command){
            ...
            case 'newmtl': // Read Material chunk
                ...
                continue; // Go to the next line
            case 'Kd':   // Read diffuse color
                if(name == "") continue; // Go to the next line because of Error
                var material = mtl.parseRGB(sp, name);
                mtl.materials.push(material);
                name = "";
                continue; // Go to the next line
            case 'map_Kd': // Read diffuse texture
                texFileName = mtl.parseDiffuseTex(sp);    // Get name
                //alert(texFileName);
                //alert("ok");
                continue; // Go to the next line
        }
...
OBJDoc.prototype.findColor = function(name){
    ...
    return(new Color(0.8, 0.8, 0.8, 1)); // defult color if material color not found
}

//------------------------------------------------------------------------------
// Retrieve the information for drawing 3D model
OBJDoc.prototype.getDrawingInfo = function() {
    // Create an arrays for vertex coordinates, normals, textures, colors, and indices
    var numIndices = 0;
...
MTLDoc.prototype.parseDiffuseTex = function(sp) {
    return sp.getWord();         // Get name
}
