...
window.onload = function init() {
    ...
    // if you create a VBO of a max size for your application,
    // specify the size in the 2nd parameter
    // do not need to send in the actual data for now
    ...
    canvas.addEventListener("mousedown", function(event){
                            ...
                            first = false;
                            gl.bindBuffer( gl.ARRAY_BUFFER, vBuffer)
                            t1 = vec2(2*(event.clientX - event.target.getBoundingClientRect().left)/canvas.width - 1,
                                      2*(canvas.height - (event.clientY - event.target.getBoundingClientRect().top))/canvas.height - 1);
                            }
                            else {
                            first = true;
                            t2 = vec2(2*(event.clientX - event.target.getBoundingClientRect().left)/canvas.width - 1,
                                      2*(canvas.height - (event.clientY - event.target.getBoundingClientRect().top))/canvas.height - 1);
                            t3 = vec2(t1[0], t2[1]);
                            t4 = vec2(t2[0], t1[1]);
                            
                            // now you send in the real data
                            // using gl.bufferSubData you are able to update
                            // only part of the data in the VBO efficiently
